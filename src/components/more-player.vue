<template>
  <div class="video-content" ref="videoMian">
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const videoMian: any = ref(null);
    const option: any = ref(null);
    //主题颜色
    onMounted(() => {
      nextTick(() => {
        //容器样式
        videoMian.value.style.height = props.height;
        videoMian.value.style.width = props.width;
    ...
    watch(
      ...
      (newVal) => {
        if (newVal != "") {
          console.log(state.acModel.active);
    ...
    const capture = (acIndex: number) => {
      ...
      let name = "截图";
      if (state.equipmentArr.length != 0 && state.equipmentArr[acIndex]?.name) {
